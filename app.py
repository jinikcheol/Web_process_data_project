...
def Monitoring():
    if not 'ID' in session:
        return ''' <script> location.href = "http://3.35.208.236:5000/" </script> '''
    else:
...
def index():
    if 'ID' in session:
        return ''' <script> location.href = "http://3.35.208.236:5000/Home" </script> '''
    else:
    ...
    if request.method == 'POST':
        ...
        if app.cnt == 5:
            return ''' <script> alert("로그인 {}회 실패 하셨기에 보안을 위해 로그인 시스템을 종료합니다 ");
                                                          location.href = "http://3.35.208.236:5000/" </script> '''\
                .format(app.cnt)

        if not row_count:
            app.cnt += 1
            return ''' <script> alert("아이디를 확인하여 주십시오");
                            location.href = "http://3.35.208.236:5000/login" </script> '''
        if row_count > 0:
            ...
            if is_pw:
                ...
                location.href = "http://3.35.208.236:5000/" </script> '''.format(id)
            else:
                app.cnt += 1
...
def logout():
    ...
    location.href = "http://3.35.208.236:5000/logo" </script> ''' % escape(session['ID'])
